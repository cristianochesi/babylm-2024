...
from tokenizer_MorPiece import MorPiece as MoP
...
def train(model, dataloader, optimizer, criterion, device, scheduler=None, clip_grad_norm=1.0):
    ...
    progress_bar = tqdm(dataloader, desc="Training", miniters=100000)
    ...
    for inputs, targets in progress_bar:
        ...
        accuracy = calculate_accuracy(output.view(-1, output.size(-1)), targets.view(-1))  # Calculate accuracy
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(model_name, "best_model.pth"))
        else:
